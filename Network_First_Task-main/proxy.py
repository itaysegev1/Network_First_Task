...
def process_request(request: api.CalculatorHeader, server_address: tuple[str, int]) -> tuple[
    ...
    doesn't allow us to use the cache, and we send a new request to the server. If the cache[request].cache_control
    is 0, the response must not be cached.
    """
    ...
    with client_socket:  # closes the socket when the block is exited
        ...
        while True:
            ...
            try:
                ...
                print(
                    f"server time remaining: {server_time_remaining:.2f}, client time remaining:"
                    f" {client_time_remaining:.2f}")
